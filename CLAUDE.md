...
**Saving placed images**: the "画像を保存" button zips every non-empty `cellImages` entry (via JSZip, loaded from a CDN `<script>` tag — the one external JS dependency in the app) and downloads it as `card-images_<timestamp>.zip`, one file per cell named by its 1-based cell number (e.g. `01.png`) with the extension taken from the blob's MIME type. This exists because `cellImages` only holds `URL.createObjectURL` blob URLs, which are lost on reload — the zip is the way to get placed images back out as real files.
